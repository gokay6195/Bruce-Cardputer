...
static bool quickloop = false;  // FIX: limit scope to file linkage
...
void IrRead::read_signal() {
    ...
    // FIX: respect raw_mode and decode_type instead of forcing raw=true
    if (decoded && results.decode_type != decode_type_t::UNKNOWN && !raw) {
        _read_signal = true;
        // raw stays false -> parsed protocol
        display_banner();
        tft.println("CAPTURED !"); 
        delay(1000);
        display_btn_options();
    }
    else if ((decoded && results.decode_type == decode_type_t::UNKNOWN) ||
             (!decoded && results.rawlen > 20)) {
        _read_signal = true;
        raw = true;  // force raw for unknown or non-decoded signals
        signals_read++;
        display_banner();
...
String IrRead::parse_state_signal() {
    String r = "";
    // FIX: round up to avoid losing the last byte when bits are not a multiple of 8
    uint16_t state_len = (results.bits + 7) / 8;
...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    // FIX: inverted condition - create directory only if it does NOT exist
